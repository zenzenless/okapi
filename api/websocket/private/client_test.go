...
// 	c, err := NewPrivateClient(okapi.AwsPublicWsURL)
...
func TestLogin(t *testing.T) {
	_, err := NewPrivateClient(okapi.DemoPrivateWsURL, rest.GetApiKeyFromEnv())
	assert.NilError(t, err)
...
func TestPositionChannel(t *testing.T) {
	c, err := NewPrivateClient(okapi.DemoPrivateWsURL, rest.GetApiKeyFromEnv())
	assert.NilError(t, err)
	positionCh, err := c.SubscribePositionChannel(&pr.Position{
		InstID:   "BTC-USDT-SWAP",
		InstType: okapi.SwapInstrument,
	})

	assert.NilError(t, err)

	assert.NilError(t, err)
	for v := range positionCh {
		fmt.Println(v)
	}
...
func TestWsBalance(t *testing.T) {
	c, err := NewPrivateClient(okapi.DemoPrivateWsURL, rest.GetApiKeyFromEnv())
	assert.NilError(t, err)
	sub, err := c.SubscribeAccountChannel(&pr.Account{
		Ccy: "USDT",
	})
	assert.NilError(t, err)
	fmt.Println(<-sub)
...
func TestGeneric(t *testing.T) {
	c, err := NewPrivateClient(okapi.DemoPrivateWsURL, rest.GetApiKeyFromEnv())
	assert.NilError(t, err)
	a, err := c.SubscribeAccountChannel(&pr.Account{Ccy: "USDT"})
...
func TestOrderList(t *testing.T) {
	c, err := NewPrivateClient(okapi.DemoPrivateWsURL, rest.GetApiKeyFromEnv())
	assert.NilError(t, err)
	a, err := c.SubscribeOrderListChannel(&pr.Order{InstID: "BTC-USDT-SWAP", InstType: okapi.SwapInstrument})
...
func TestAlgoOrderList(t *testing.T) {
	c, err := NewPrivateClient(okapi.DemoPrivateWsURL, rest.GetApiKeyFromEnv())
	assert.NilError(t, err)
	a, err := c.SubscribeAlgoOrderList(&pr.AlgoOrder{InstID: "BTC-USDT-SWAP", InstType: okapi.SwapInstrument})
...
func TestAlgoAdvanceOrderList(t *testing.T) {
	c, err := NewPrivateClient(okapi.DemoPrivateWsURL, rest.GetApiKeyFromEnv())
	assert.NilError(t, err)
	a, err := c.SubscribeAlgoAdvanceList(&pr.AlgoOrder{InstID: "BTC-USDT-SWAP", InstType: okapi.SwapInstrument})
