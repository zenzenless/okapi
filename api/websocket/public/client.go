...
func NewPublicClient(addr okapi.BaseURL) (*Client, error) {
	...
	if !(addr == okapi.AwsPublicWsURL || addr == okapi.PublicWsURL || addr == okapi.DemoPublicWsURL) {
		return nil, fmt.Errorf("this is public websocket client ,should use public url, your url :%s", addr)
	}
	c, err := wsclient.NewClient(addr, publicArgToID)
	return &Client{
		Client: c,
	}, err
}
...
// func NewPublicClient(addr okapi.BaseURL, apiKeys ...*rest.APIKey) (*Client, error) {
